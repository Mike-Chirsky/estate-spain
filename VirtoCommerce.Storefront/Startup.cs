    ...
    public class Startup
    {
            ...
            var distributedCache = CacheFactory.Build("distributedCache", settings =>
      {
          ...
          if (redisCacheEnabled)
          {
            ...
            container.RegisterType<IPricingModuleApiClient>(new PerRequestLifetimeManager(), new InjectionFactory(c => new PricingModuleApiClient(baseUri, c.Resolve<VirtoCommerceApiRequestHandler>(), c.Resolve<ILocalCacheManager>(), c.Resolve<System.Net.Http.HttpClientHandler>()).DisableRetries().WithTimeout(apiRequestTimeout)));
            ...
            container.RegisterInstance<ICategoryTreeService>(new ESCategoryTreeService(container.Resolve<ICatalogModuleApiClient>(), workContextFactory));
